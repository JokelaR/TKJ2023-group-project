...
const uint16_t emask = 0b1111000000000000;
const uint16_t rmask = 0b0000111111111111;
...
double opt3001_get_data(I2C_Handle *i2c) {
	...
	if (opt3001_get_status(i2c) & OPT3001_DATA_READY) {
		...
		if (I2C_transfer(*i2c, &i2cMessage)) {
		    uint16_t readBuffer = rxBuffer[0] << 8;
		    readBuffer = readBuffer | rxBuffer[1];

	        // JTKJ: Here the conversion from register value to lux
            uint16_t e = (readBuffer & emask) >> 12;
            uint16_t r = readBuffer & rmask;
            lux = 0.01 * pow(2, e) * r;
